...
- `SUPABASE_SECRET_KEY` 只能存在 Nitro server runtime / Vercel server env，不可進入 public runtime config。
...
本 repo 目前使用以下變數名稱。部署平台必須使用這些名稱：

| 變數                  | Staging 來源                       | Browser 可見 | 備註                                                          |
| --------------------- | ---------------------------------- | ------------ | ------------------------------------------------------------- |
| `SUPABASE_URL`        | Supabase staging Project URL       | 是           | 例如 `https://<project-ref>.supabase.co`                      |
| `SUPABASE_KEY`        | Supabase anon / publishable key    | 是           | client auth 與一般 authenticated request 使用                 |
| `SUPABASE_SECRET_KEY` | Supabase service role / secret key | 否           | server-only elevated key                                      |
| `NUXT_PUBLIC_APP_URL` | Vercel staging / preview URL       | 是           | 必須是 `https://...`，會影響 Supabase auth cookie secure 判斷 |
...
不要使用以下錯名，除非未來同步修改程式碼與文件：

- `SUPABASE_ANON_KEY`
- `SUPABASE_SERVICE_ROLE_KEY`
- `NUXT_PUBLIC_SITE_URL`

目前 [nuxt.config.ts](../nuxt.config.ts) 會讀取 `SUPABASE_KEY`，並相容 fallback `SUPABASE_PUBLISHABLE_KEY`；部署時仍以 `SUPABASE_KEY` 作為標準名稱。

...
begin
  if p_lock then
    perform pg_advisory_xact_lock(hashtext('work_order_test_paper_order_no'), 99);
  end if;

  select max(substring(work_orders.paper_order_no from 3)::integer)
  into v_max_sequence
  from public.work_orders
  where paper_order_no ~ '^99[0-9]+$';
...
begin
  ...
  v_paper_order_mode := coalesce(nullif(trim(p_work_order ->> 'paperOrderMode'), ''), 'standard');

  if v_paper_order_mode = 'standard' then
    v_paper_order_no := public.get_next_admin_paper_order_no(true);
  elsif v_paper_order_mode = 'test' then
    perform pg_advisory_xact_lock(hashtext('work_order_test_paper_order_no'), 99);
    v_paper_order_no := trim(coalesce(p_work_order ->> 'paperOrderNo', ''));

    if v_paper_order_no !~ '^99[0-9]{4,}$' then
      raise exception 'Invalid test paper order number'
        using errcode = '23514';
    end if;
  else
    raise exception 'Invalid paper order mode'
      using errcode = '23514';
  end if;
...
revoke execute on function public.get_next_admin_test_paper_order_no(boolean) from anon;
grant execute on function public.get_next_admin_test_paper_order_no(boolean) to authenticated;

...
- 最後更新：2026-06-27
- 目前階段：Cloud-to-Physical Printing MVP 已完成；LINE 官方帳號串接 12 PR主線已完成並進入release verification。Staging deploy、signed webhook、processor auth、負向job語意與Vault/Cron已驗證；LIFF token redirect、login redirectUri、click-time token preservation、single order-gate URL builder、DOM click navigation diagnostics、debug-only hash id token fallback、LIFF logged-in mismatch diagnostics 與 mounted primary redirect init hotfix 已完成；production真人LIFF confirm、綁定落庫、`line_binding_success` processor與實際 LINE push均已驗證；confirm成功後的 best-effort immediate drain已加入；order-gate綁定按鈕會在已登入 LIFF 後 best-effort 呼叫 `liff.requestFriendship()` 引導加入官方帳號且不阻擋綁定；order-gate debug block已收斂為 `debug=1` 專用，非 debug 顧客流程與成功頁不顯示診斷區；production Supabase Cron已啟用並確認最近三輪每分鐘HTTP 200；production READY push已送達且 `notified_at` 已寫入；已綁定老客建立新工單會 enqueue `work_order_received` 並由Cron送達；production follow/unfollow webhook 已驗證可更新好友狀態；三種 LINE 自動通知已改為使用 repo 內 Flex Message 模板。
- 整體狀態：進行中
- 現況摘要：
  ...
  - LINE release hardening結果記錄於 `docs/line-release-verification.md`：staging migrations同步、Vercel env補齊、production webhook active、staging follow/unfollow簽章E2E、processor 401/200、skipped/failed不寫`notified_at`及每分鐘Cron HTTP 200均已驗證。真人測試發現 LIFF root query URL `https://liff.line.me/{LIFF_ID}/?t=...` 在 LINE iOS 入口會被改寫成 bare `?t` 並遺失 `debug=1`；repo 已改用 `https://liff.line.me/{LIFF_ID}/t/{token}`，由 LINE Developers endpoint `/line/order-gate` 承接，支援解析 `liff.state`、`/t/{token}` path 與 secondary `/line/order-gate/t/{token}` canonical route。order-gate 在 mounted 階段先偵測 LIFF primary redirect：若同時存在 `liff.state` 與 hash 內 `access_token` / `id_token`，先顯示「正在初始化 LINE...」並呼叫 `liff.init()`，不先打 resolve API、不顯示 pending 綁定按鈕；若 SDK未自動完成secondary redirect，才用 `buildOrderGateUrl(token, debug)` 以非空 token導到 canonical `/line/order-gate?t={token}`。click handler 仍固定使用已 resolve 成功的 `resolvedToken`，並保留 click-time LIFF step、masked hash token diagnostics、`debug=1&dryRun=1` 無跳頁診斷與 DOM click debug欄位；debug block現在只在明確 `debug=1` 時顯示，非 debug 頁會清掉舊 click debug snapshot，成功 / already linked狀態不顯示診斷區。已登入 LIFF 後會 best-effort 呼叫 `liff.requestFriendship()`，成功、取消、失敗或不支援都繼續原本 confirm binding。production真人測試已確認 confirm 200、token used、binding落庫、`line_binding_success` job送出成功且官方 LINE實際收到綁定成功通知。confirm `linked` outcome現在會 best-effort 立即 drain既有 processor以降低綁定成功通知延遲；失敗不改變 confirm response。Production Supabase Cron已安裝 `pg_cron` / `pg_net`，Vault保存 processor URL與secret，每分鐘 schedule已 active，最近三輪 `cron.job_run_details` succeeded且 `net._http_response` 皆回HTTP 200。READY push已由Cron觸發送達，補上 `service_role` 對 `work_orders` 的 UPDATE grant後，`work_order_ready_for_pickup` job已是 `succeeded` 且 `notified_at` 已寫入；老客新工單 `260002` 的 `work_order_received` job已由Cron處理為 `succeeded` 並確認顧客端收到收件通知。production follow/unfollow webhook已以真實LINE封鎖/解除封鎖驗證：封鎖會寫入 `blocked_at` / `friendship_checked_at`，解除封鎖會回到 `is_friend=true` 並清空 `blocked_at`。三種 LINE 自動通知目前使用 `line-flex-message/*.json` 作為 Flex Message 模板來源，processor 會用實際工單號替換模板測試單號並凍結到 `prepared_messages`。
  ...
  - LINE bind token service 已建立 server-only HMAC / SHA-256 helpers、LIFF URL 重建、token 狀態解析與原子 issue / revoke RPC。DB 只保存 hash；Admin API 與 Public LIFF API 均已使用此服務。LIFF URL 目前使用 `https://liff.line.me/{LIFF_ID}/t/{token}`，避免 root query 被 LINE iOS 入口改寫成 bare `?t`，也避免與 LINE Developers endpoint `/line/order-gate` 重複拼接。
  - Admin LINE API 已建立：工單重新發卡、Customer 解除綁定與工單 LINE 狀態查詢。解除綁定會 hard delete binding、撤銷 pending token並 skip pending jobs；工單詳情頁已接上 LINE 狀態卡、發卡、複製 LIFF 連結與解除綁定確認操作。

...
- 最後更新：2026-04-29
- 目前階段：admin 工單列表與詳情頁已有第一版雛形，下一步持續補 dashboard live data、建單頁、detail 的 work mutation，並在與甲方討論後整理前端第二版細節調整
- 整體狀態：進行中
- 現況摘要：
  ...
  - `/`、`/login`、`/admin`、`/forbidden` 已重整到 Tailwind/shadcn 基礎；`/admin/work-orders` 已可查詢、篩選、排序與分頁，`/admin/work-orders/[id]` 已提供 `mode=view|edit|work` detail route，且 `mode=edit` 已接上 PATCH。
...
- 依既有 admin API 補 admin dashboard live data、工單建單頁與 detail 的 `work` mode。
...
- 實作 admin 工單建單頁面，接上既有 work-order API。
...
- Admin 前端目前已有 Tailwind/shadcn shell、工單列表與 detail 的 `view/edit` 第一版，但 dashboard 仍是 placeholder，detail 的 `work` mutation 與 create 仍待實作。

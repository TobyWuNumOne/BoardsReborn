...
驗證狀態：repo tests/build 已通過；仍需部署到 production 後，以新發卡 token 重新跑真人 LIFF 綁定。預期第一次 LINE App landing 會先顯示 primary 初始化 debug，之後才進入 `/line/order-gate?t={token}` 的 secondary resolve / pending 畫面。

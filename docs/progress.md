...
- 最後更新：2026-06-25
- 目前階段：Cloud-to-Physical Printing MVP 已完成；LINE 官方帳號串接 12 PR主線已完成並進入release verification。Staging deploy、signed webhook、processor auth、負向job語意與Vault/Cron已驗證；LIFF token redirect、login redirectUri、click-time token preservation、single order-gate URL builder、DOM click navigation diagnostics、debug-only hash id token fallback、LIFF logged-in mismatch diagnostics 與 mounted primary redirect init hotfix 已完成；production真人LIFF已走到 confirm API，且 confirm 500 root cause已定位為 service_role缺少 `customers` row-lock所需的 `UPDATE` privilege；已套用 grant migration，待重新測試真人LIFF綁定，production Cron未啟用。
- 整體狀態：進行中
- 現況摘要：

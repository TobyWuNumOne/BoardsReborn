...
實際安裝解析以 `package.json` 與 `pnpm-lock.yaml` 為準。若要升級 Nuxt、Nitro、Node 或格式化工具，請同步更新本段、重新執行 `pnpm install`，並至少跑 `pnpm format:check`、`pnpm lint`、`pnpm test`、`pnpm typecheck`、`pnpm build`。

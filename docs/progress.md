...
- 最後更新：2026-04-22
- 目前階段：第一批 admin work-order API 已建立，準備補狀態與列印流程
- 整體狀態：進行中
- 現況摘要：
  ...
  - `server/api/` 已有 admin customer lookup 與 work-order create/list/detail/update handlers。
  ...
  - Admin auth helper 已能檢查 Supabase cookie session 與 `admin_profiles` gate；完整登入 UI / session flow、status update API、print-agent implementation 與 production workflow 仍未建立。
...
| Admin work-order API                   | partial | Create/list/detail/update 與 customer lookup 已建立；status、bulk、print 仍未實作。 |

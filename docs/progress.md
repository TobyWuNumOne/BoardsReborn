...
- 最後更新：2026-06-23
- 目前階段：Cloud-to-Physical Printing MVP 已完成；LINE 官方帳號串接 12 PR主線已完成並進入release verification。Staging deploy、signed webhook、processor auth、負向job語意與Vault/Cron已驗證；LIFF token redirect、login redirectUri、click-time token preservation、single order-gate URL builder、DOM click navigation diagnostics、debug-only hash id token fallback 與 LIFF logged-in mismatch diagnostics hotfix 已完成，authenticated admin、重新發卡後的真實LIFF綁定及真實LINE push仍待人工驗收，production Cron未啟用。
- 整體狀態：進行中
- 現況摘要：
  ...
  - LINE release hardening結果記錄於 `docs/line-release-verification.md`：staging migrations同步、Vercel env補齊、production webhook active、staging follow/unfollow簽章E2E、processor 401/200、skipped/failed不寫`notified_at`及每分鐘Cron HTTP 200均已驗證。真人測試發現 LIFF query / path 在開啟與 `liff.init()` secondary redirect 中容易遺失或重複；repo 已改用 `https://liff.line.me/{LIFF_ID}/?t=...`，由 LINE Developers endpoint `/line/order-gate` 承接，支援解析 `liff.state`，並以 absolute `/line/order-gate?t=...` 建立 login redirectUri。最新 hotfix 讓 click handler 固定使用已 resolve 成功的 `resolvedToken`，補上 `?debug=1` click-time LIFF step、masked hash token diagnostics與 `debug=1&dryRun=1` 無跳頁診斷；正式流程仍優先 `liff.init()` / `liff.getIDToken()`，只有 debug 模式且 SDK 回報 missing token或 logged-in mismatch時，才用 URL hash 的 LIFF `id_token` 作 fallback 送 confirm API，由後端驗證 ID token。order-gate URL 已收斂到單一 `buildOrderGateUrl()` helper，computed redirectUri與實際 `liff.login` redirectUri一致，並新增 actual redirect / explicit navigation debug 欄位；repo search確認 order-gate頁面沒有顯式導向空 token URL，pending綁定按鈕也未包在 link/form中。按鈕已加 `@click.prevent.stop` 與 DOM click debug 欄位，方便部署後確認是否仍有原生導頁介入。仍需部署後重新發卡驗證真人LIFF與成功push。
  ...
  - LINE bind token service 已建立 server-only HMAC / SHA-256 helpers、LIFF URL 重建、token 狀態解析與原子 issue / revoke RPC。DB 只保存 hash；Admin API 與 Public LIFF API 均已使用此服務。LIFF URL 目前使用 `https://liff.line.me/{LIFF_ID}/?t=...`，避免與 LINE Developers endpoint `/line/order-gate` 重複拼接。

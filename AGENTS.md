...
- Supabase local config, migrations, seed placeholder, generated Database types, server API foundation helpers, admin/public lookup handlers, surfboard `board_length_class` support in admin create/list/detail, Tailwind/shadcn frontend foundation, and minimal login/session UI exist; print-agent implementation is still pending.
...
- Use [docs/frontend.md](docs/frontend.md) for admin frontend route, layout, styling, component, form, and UI state decisions.
...
- Customer progress lookup must go through server API validation with paper order number and full normalized phone number. A wrong phone value should return `404`.
...
- Current frontend toolchain baseline is documented in [docs/setup.md](docs/setup.md); keep it synchronized when changing Node, pnpm, Nuxt, Nitro, Vue, TypeScript, ESLint, Prettier, or Vitest versions.

...
本輪驗證 staging、production 啟用前條件與 production Cron 啟用結果；不新增功能、不改 LINE 架構。

## 驗證環境

- 驗證日期：2026-06-22 至 2026-06-26
...
- Production Supabase project已於 2026-06-26 建立 Vault secrets 與每分鐘 Cron。
...
| LIFF URL可開啟                | PASS | `/t/{token}` LIFF URL已完成真人LINE App login / bind；root query格式已棄用               |
...
- [x] 部署 LIFF token preservation hotfix 後，重新發卡並確認 `/t/{token}` URL可完成真人綁定，不再使用會產生 bare `?t` 的 root query格式。
...
- [x] READY更新成功時確認`lineNotification` toast。
- [x] 在LINE App開啟LIFF，完成login與實際綁定。
- [ ] 在LINE Developers確認LINE Login/LIFF channel與Messaging API channel位於同一Provider。
- [x] 綁定後確認後台狀態、display name與好友狀態更新。已確認binding、display name，以及production follow/unfollow會更新 `is_friend`、`blocked_at`、`friendship_checked_at`。
- [x] 以真實可通知LINE user執行READY push，確認手機收到訊息、job succeeded及`notified_at`設定。
- [x] 驗證真實follow/unfollow事件由LINE Platform送達production webhook。
...
- [x] Production Vault建立processor URL與secret。
- [x] Production Cron建立前再次manual invoke processor。
- [x] Production Cron啟用後觀察run details與HTTP response。

2026-06-26已安裝production `pg_cron` / `pg_net`，建立 `line_job_processor_url` 與 `line_job_processor_secret` Vault secrets，並建立每分鐘 `boardsreborn-line-jobs-every-minute` schedule。已觀察最近三輪 `cron.job_run_details` 皆為 succeeded，`net._http_response` 皆回 HTTP 200 與 processor summary `{ claimed: 0, failed: 0, retried: 0, skipped: 0, succeeded: 0 }`。

### End-to-End

- [ ] Authenticated admin UI smoke全部通過。
- [x] 真實LIFF綁定流程通過。
- [x] 真實follow/unfollow webhook通過。
- [x] 真實READY push送達、job succeeded及`notified_at`通過。
- [x] 真實老客新工單收件通知送達，`work_order_received` job succeeded。
- [ ] 確認failed / skipped不寫`notified_at`。

## Release Gate

Production Cron已啟用並初步驗證 HTTP 200。只有剩餘人工驗收待辦與production checklist全部完成後，才能將LINE MVP狀態改為正式上線完成。
...
- Debug block 只在 URL / `liff.state` / `liff_state` / current URL fallback 明確包含 `debug=1` 時顯示；一般顧客流程不因舊 `sessionStorage` debug event 顯示診斷區。非 debug 頁會清除舊 click debug snapshot，綁定成功 / already linked 狀態也不顯示 debug block。
...
- 後續 production真人測試確認 click flow已完整走到 `POST /api/public/line-bind/confirm`，`liff.init()`為 initialized、`isLoggedIn=true`、`confirmApiCalled=true`，不再是 token URL / click handler問題；目前回 `500 INTERNAL_SERVER_ERROR`。
- 2026-06-24 只讀檢查 production Supabase project `ystpdyihgpecmadlnydj`，確認 LINE foundation、confirm、webhook、processor、READY與received notification migrations均已存在，`confirm_public_line_binding(...)` function signature也存在。Vercel production project也確認 LINE env key名稱已存在，且最新 production deployment晚於 env建立時間。
- 因 Vercel runtime log目前只顯示 `/api/public/line-bind/confirm 500`，未包含 Supabase RPC error detail；repo已補 server-side sanitized diagnostic log。下一次 production retest若仍回500，Vercel Runtime Logs應可看到 `LINE binding confirm RPC failed` 及 Supabase `code/message/details/hint`，其中 plaintext token、token hash與 LINE user id會被遮罩。

驗證狀態：repo tests/build 已通過；仍需部署到 production 後，以新發卡 token 重新跑真人 LIFF 綁定。預期新發卡 LIFF URL 會長成 `https://liff.line.me/{LIFF_ID}/t/{token}`；第一次 LINE App landing 會先顯示 primary 初始化 debug或經由 `/line/order-gate/t/{token}` canonical route，之後才進入 `/line/order-gate?t={token}` 的 secondary resolve / pending 畫面。若 confirm仍回500，先抓 Vercel Runtime Logs中的 sanitized `LINE binding confirm RPC failed` 再判斷是否需要 DB function hotfix。

## 2026-06-25 Production LIFF Confirm 500

Production真人測試已確認 `/line/order-gate/t/{token}/debug` 可正確解析 token，`liff.init()`為 initialized、`isLoggedIn=true`、click handler會呼叫 `POST /api/public/line-bind/confirm`。目前失敗點已不在LIFF URL、token extraction或按鈕事件，而是 server-side confirm transaction。

Vercel Runtime Logs顯示：

- `LINE binding confirm RPC failed`
- Supabase error code：`42501`
- message：`permission denied for table customers`
- hint：`GRANT UPDATE ON public.customers TO service_role;`

Root cause：`confirm_public_line_binding` 以 `FOR UPDATE` 鎖定 target Customer row。Postgres即使沒有修改 customer欄位，也要求呼叫 role具備該 table的 `UPDATE` privilege。production service role只有 select，導致 confirm RPC在鎖 row時失敗。

Repo hotfix：新增 migration `20260625110031_line_confirm_service_role_customer_lock_grant.sql`，補上：

```sql
grant update on table public.customers to service_role;
```

下一步：已將此 migration套到 production，並確認 `service_role` 對 `public.customers` 具備 `UPDATE` privilege。接著以新發卡 token重新跑真人LIFF綁定；若仍回500，優先查 Vercel Runtime Logs中的 sanitized `LINE binding confirm RPC failed`。

## 2026-06-26 Production Binding Success Push

Production真人測試已確認新發卡 `/line/order-gate/t/{token}/debug` 流程可完成 LIFF confirm：

- confirm API回 `200`，debug顯示 `after_confirm_api`。
- 最新 bind token已寫入 `used_at`。
- `customer_line_accounts` 已建立。
- `line_binding_success` job已建立，manual invoke production processor後變為 `succeeded`。
- 顧客端實際收到「LINE 通知綁定成功。工單：260001」。

Repo follow-up：confirm `linked` outcome後新增 best-effort immediate drain，直接重用既有 `processLineJobs()` 以降低 `line_binding_success` 通知延遲。此機制失敗不改變 confirm response，且不取代 production Supabase Cron；Cron仍需作為 retry與漏送補償機制啟用。

## 2026-06-26 Production READY Push

Production Cron啟用後，以工單 `260001` 進入 `READY_FOR_PICKUP` 測試真實 READY 通知。顧客端已收到完工待取件訊息，表示 LINE push call 已成功。

首次 result回寫時發現 `line_jobs` 停在 `locked`，`work_orders.notified_at` 仍為 null。Root cause：`record_line_job_result` 成功分支會更新 `work_orders.notified_at`，production `service_role` 缺少 `public.work_orders` 的 `UPDATE` privilege，導致 result transaction rollback。已補 repo migration `20260626214144_line_ready_notification_service_role_work_orders_grant.sql` 並於 production 執行：

```sql
grant update on table public.work_orders to service_role;
```

修正後以既有 `record_line_job_result` RPC 回寫該筆已送達的 READY job，不再呼叫 LINE API。最終狀態：`work_order_ready_for_pickup` job為 `succeeded`，`locked_at` / `locked_by` 已清空，`work_orders.notified_at` 已寫入。

## 2026-06-26 Production Received Push

Production Cron啟用後，以已綁定 Customer 建立新工單測試 `work_order_received` 通知：

- 新工單 `260002` 建立後產生 `work_order_received` line job。
- Cron約30秒後處理該 job，狀態變為 `succeeded`。
- 顧客端實際收到收件通知。
- 該 job `locked_at` 已清空，`attempts=1`，`sent_at` 已寫入。

這確認「老客已綁定後建立新工單會收到收件通知」的 production E2E 流程已通過。此通知仍走Cron保底，不要求建單API等待LINE push成功。

## 2026-06-26 Production Friendship State

Production `customer_line_accounts` 初始只讀檢查顯示最新綁定：

- `is_friend=false`
- `blocked_at=null`
- `friendship_checked_at=null`

在目前schema語意下，這代表「好友狀態尚未被follow/unfollow webhook或friendship check確認」，不是已確認封鎖。已驗證該LINE帳號仍可收到 `line_binding_success`、`work_order_ready_for_pickup` 與 `work_order_received` 三種通知。

後續以真實LINE帳號測試 production webhook：

- 封鎖官方帳號後，DB更新為 `is_friend=false`，並寫入 `blocked_at=2026-06-26 14:23:09.843+00` 與 `friendship_checked_at=2026-06-26 14:23:09.843+00`。
- 解除封鎖後，DB更新為 `is_friend=true`、`blocked_at=null`，並寫入 `friendship_checked_at=2026-06-26 14:23:38.169+00`。

結論：production follow / unfollow webhook E2E通過；後台好友狀態可被真實LINE事件更新。

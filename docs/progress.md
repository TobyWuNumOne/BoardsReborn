...
- 現況摘要：
  ...
  - Admin auth helper 已能檢查 Supabase cookie session 與 `admin_profiles` gate；完整登入 UI / session flow、bulk status API、print-agent implementation 與 production workflow 仍未建立。

...
- Public confirm由 Nuxt service-role client呼叫；因 `confirm_public_line_binding` 會以 `FOR UPDATE` 鎖定 `customers` row，`service_role` 需要 `customers` 的 `UPDATE` privilege，即使 function不修改 customer欄位。

...
- Production dry-run 進一步定位到 click handler token與 endpoint redirectUri皆正確，最後停在 `before_liff_init`，因此新增 hash id token fallback：若 `window.location.hash` 已含 LIFF `id_token`，click handler 不呼叫 `liff.init()` / `liff.login()`，直接把 `resolvedToken + idToken + optional accessToken` 送 confirm API，由 server 端照既有規格驗證 ID token。
- hash token fallback 只在函式區域 / transient memory 讀取 `id_token` / `access_token`，不寫入 debug block、localStorage或sessionStorage，不解析 profile或LINE user id。
